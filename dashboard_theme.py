...
function drawTrend(containerId, country, brands=BRAND_ORDER, height=230){
  ...
  const dates = datesFor(country, 7);
  ...
  dates.forEach((d,i)=>svg+=`<text x="${x(i)}" y="${h-8}" text-anchor="middle" font-size="11" fill="#94A3B8">${d.slice(5)}</text>`);
  series.forEach(s=>{
...
function refreshTrendOptions(countries){
...
function renderBigTrend(){
  const c = byId("trendCountry").value || GROUPS.focus[0];
